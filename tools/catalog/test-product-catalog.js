...
assert(activeProducts.every(product => !product.image_file_id), 'public snapshot must not contain deployment-specific cloud images')
assert(activeProducts
  .filter(product => product.title !== '祝福项坠')
  .every(product => product.image_url.startsWith('https://')), 'active products should use public image URLs when available')
...
const publicImagePrefix = 'https://wiki.biligame.com/'
assert(goodsInfo.getGoodsImage('血脉秘药').startsWith(publicImagePrefix))
assert(goodsInfo.getGoodsImage('粉尘').startsWith(publicImagePrefix))
assert(goodsInfo.getGoodsImage('矿石').startsWith(publicImagePrefix))
...
assert(ordinaryDust.image.startsWith(publicImagePrefix))
assert(ordinaryDust.imageCandidates[0].startsWith(publicImagePrefix))
const ordinaryDustFallback = products.advanceProductImage({ ...ordinaryDust, hasImage: true })
assert.strictEqual(ordinaryDustFallback.hasImage, false)
assert(products.matchProductByName('光合球').image.startsWith(publicImagePrefix))
const sourceCloudImage = 'cloud://test-environment/source-priority-test.png'
const attachedBloodPotion = goodsInfo.attachGoodsMeta({ name: '火系血脉秘药', image: sourceCloudImage })
assert.strictEqual(attachedBloodPotion.image, sourceCloudImage)
assert(attachedBloodPotion.imageCandidates.some(image => image.startsWith(publicImagePrefix)))
