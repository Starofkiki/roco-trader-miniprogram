...
function getCloudConfig() {
  ...
  return {
    resourceAppid: globalData.cloudAppId || '',
...
function getCloudInstance() {
  ...
  if (!config.resourceAppid && !config.resourceEnv) {
    cloudInstance = wx.cloud
    cloudInitPromise = Promise.resolve()
    return cloudInitPromise.then(() => cloudInstance)
  }
  const cloudOptions = {}
  if (config.resourceAppid) cloudOptions.resourceAppid = config.resourceAppid
  if (config.resourceEnv) cloudOptions.resourceEnv = config.resourceEnv
  cloudInstance = new wx.cloud.Cloud(cloudOptions)
